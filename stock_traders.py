...
class MyMaTrader(StockTrader):
    def init_selectors(self, region: Region, entity_ids, entity_schema, exchanges,
                       codes, start_timestamp, end_timestamp, adjust_type=None):

        myselector = TargetSelector(region=region, entity_ids=entity_ids,
                                    entity_schema=entity_schema, exchanges=exchanges,
                                    codes=codes, start_timestamp=start_timestamp,
                                    end_timestamp=end_timestamp,
                                    provider=Provider.JoinQuant)

        myselector.add_filter_factor(
            CrossMaFactor(region=region, entity_ids=entity_ids,
                          entity_schema=entity_schema, exchanges=exchanges,
                          codes=codes, start_timestamp=start_timestamp,
                          end_timestamp=end_timestamp, windows=[5, 10],
                          need_persist=False, adjust_type=adjust_type))
...
if __name__ == '__main__':
    ...
    MyMaTrader(region=Region.CHN, codes=['000338'], level=IntervalLevel.LEVEL_1DAY, start_timestamp='2018-01-01',
