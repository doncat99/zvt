...
class Factor(DataReader, DataListener):
    ...
    def load_factor(self):
        # read state
        states: List[FactorState] = FactorState.query_data(region=self.region,
        ...
        if self.dry_run:
            ...
            if self.accumulator is not None:
                self.factor_df = self.load_window_df(provider=Provider.ZVT, data_schema=self.factor_schema,
    ...
    def clear_state_data(self, region: Region, entity_id=None):
        if entity_id:
            del_data(region,
                     ...
                     provider=Provider.ZVT)
            del_data(region, self.factor_schema, filters=[self.factor_schema.entity_id == entity_id], provider=Provider.ZVT)
        else:
            del_data(region, FactorState, filters=[FactorState.factor_name == self.factor_name], provider=Provider.ZVT)
    ...
    def persist_factor(self):
        ...
        if self.states:
            session = get_db_session(region=self.region, provider=Provider.ZVT, data_schema=FactorState)
            ...
            for entity_id in self.states:
                ...
                try:
                    ...
                    if entity_id in g.groups:
                        df_to_db(df=df.loc[(entity_id,)], ref_df=None, region=self.region, data_schema=self.factor_schema, provider=Provider.ZVT)
            ...
            df_to_db(df=df, ref_df=None, region=self.region, data_schema=self.factor_schema, provider=Provider.ZVT)
