...
class MyMaTrader(StockTrader):
    def init_selectors(self, region, entity_ids, entity_schema, exchanges, codes, start_timestamp, end_timestamp):
        ...
        myselector.add_filter_factor(
            CrossMaFactor(region=region, entity_ids=entity_ids, entity_schema=entity_schema, exchanges=exchanges,
                          codes=codes, start_timestamp=start_timestamp, end_timestamp=end_timestamp,
...
class MyBullTrader(StockTrader):
    def init_selectors(self, region, entity_ids, entity_schema, exchanges, codes, start_timestamp, end_timestamp):
        ...
        myselector.add_filter_factor(
            BullFactor(region=region, entity_ids=entity_ids, entity_schema=entity_schema, exchanges=exchanges,
                       codes=codes, start_timestamp=start_timestamp, end_timestamp=end_timestamp))
