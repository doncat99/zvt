...
from zvt.api.trader_info_api import AccountStatsReader, OrderReader, get_order_securities, get_trader_info
from zvt.domain import TraderInfo
from zvt.contract import zvt_context, IntervalLevel, Mixin
...
def load_traders(region):
    ...
    for trader in traders:
        account_readers.append(AccountStatsReader(region=region, trader_names=[trader.trader_name], level=trader.level))
        order_readers.append(
            OrderReader(region=region, trader_names=[trader.trader_name], level=trader.level, start_timestamp=trader.start_timestamp,
...
def update_trader_details(region: Region, trader_index, entity_type):
    if trader_index is not None:
        ...
        df = get_entities(region=region, entity_type=entity_type, entity_ids=entity_ids, columns=['entity_id', 'code', 'name'],
                          index='entity_id')
        ...
        account_stats = None
        df = get_entities(region=region, entity_type=entity_type, columns=['entity_id', 'code', 'name'], index='entity_id')
        entity_options = [{'label': f'{entity_id}({entity["name"]})', 'value': entity_id} for entity_id, entity in
                          df.iterrows()]
        return account_stats, entity_type_options, entity_options
...
def update_factor_details(region: Region, factor, entity_type, entity, levels, columns, trader_index, schema_name):
    if factor and entity_type and entity and levels:
        ...
        if columns:
            ...
            sub_df = schema.query_data(region=region, entity_id=entity, columns=columns)
